...
const config = (0, harness_config_1.loadHarnessConfig)();
const logger = (0, harness_config_1.createLogger)(config.logLevel);
...
const db = new sqlite3.Database(HARNESS_DB_PATH, (error) => {
    if (error) {
...
async function initializePersistenceSchema() {
    try {
        await runSql(`
...
function toConversationTitleFromFirstUserText(text) {
...
function toPersistedAssistantPayload(payload) {
    return {
...
function readTokenDerivedUserId(rawToken) {
...
function readFirstNonEmptyString(source, keys) {
...
function extractAuthToken(headers, requestUrl) {
...
function resolveUserId(headers, requestUrl) {
...
async function upsertConversationForTurn(conversationId, userId, userText) {
    await ensurePersistenceReady();
    const existing = await getSql('SELECT id, user_id, title, created_at, updated_at FROM conversations WHERE id = ?', [conversationId]);
    const timestamp = nowIso();
    const preferredTitle = toConversationTitleFromFirstUserText(userText);
    if (!existing) {
        await runSql('INSERT INTO conversations (id, user_id, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?)', [conversationId, userId, preferredTitle || 'New chat', timestamp, timestamp]);
        return;
    }
    if (existing.user_id !== userId) {
        throw new Error('Conversation is owned by another user.');
    }
    await runSql('UPDATE conversations SET updated_at = ? WHERE id = ?', [timestamp, conversationId]);
    if (existing.title === 'New chat' && preferredTitle && preferredTitle !== 'New chat') {
        const firstUserMessage = await getSql('SELECT COUNT(1) as count FROM messages WHERE conversation_id = ? AND role = ?', [conversationId, 'user']);
        const hasUserMessages = !!(firstUserMessage && Number(firstUserMessage.count) > 0);
        if (!hasUserMessages) {
            await runSql('UPDATE conversations SET title = ? WHERE id = ?', [preferredTitle, conversationId]);
        }
    }
}
async function appendUserMessage(conversationId, userId, text, hostMessageId) {
    await ensurePersistenceReady();
    const timestamp = nowIso();
    await runSql(`
...
async function appendAssistantMessage(conversationId, userId, assistantPayload) {
    await ensurePersistenceReady();
    const timestamp = nowIso();
    await runSql(`
      INSERT INTO messages (
    ...
    await runSql('UPDATE conversations SET updated_at = ? WHERE id = ? AND user_id = ?', [timestamp, conversationId, userId]);
}
...
const httpServer = (0, http_1.createServer)(async (req, res) => {
    const requestUrl = new url_1.URL(req.url || '/', 'http://localhost');
    ...
    if (req.method === 'OPTIONS') {
        ...
        const requestedList = requested.split(',').map((s) => s.trim()).filter((s) => !!s);
        const allowedList = getAllowedRequestHeaders().map((h) => h.toLowerCase());
        const missing = requestedList.filter((h) => allowedList.indexOf(h.toLowerCase()) < 0);
        const allowedAll = missing.length === 0;
        logger.info('[mock-harness] CORS preflight origin=%s path=%s requestedHeaders=%s allowedAll=%s missing=%s', requestOrigin, requestUrl.pathname, requested || '(none)', allowedAll, missing.join(',') || '(none)');
    ...
    if (requestUrl.pathname === '/v1/conversations' && req.method === 'GET') {
        ...
        `, [userId]);
            ...
            logger.error('[mock-harness] failed to list conversations', error);
    ...
    if (requestUrl.pathname === '/v1/conversations' && req.method === 'POST') {
        ...
        try {
            ...
            const title = body && typeof body.title === 'string'
                ...
                : 'New chat';
            const existing = await getSql('SELECT id, user_id, title, created_at, updated_at FROM conversations WHERE id = ?', [requestedConversationId]);
            const timestamp = nowIso();
            if (!existing) {
                await runSql('INSERT INTO conversations (id, user_id, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?)', [requestedConversationId, userId, title || 'New chat', timestamp, timestamp]);
            }
                ...
                await runSql('UPDATE conversations SET title = ?, updated_at = ? WHERE id = ? AND user_id = ?', [title || existing.title || 'New chat', timestamp, requestedConversationId, userId]);
            }
            sendJson(res, 201, { id: requestedConversationId, title: title || 'New chat' }, corsHeaders);
            return;
        }
        catch (error) {
            logger.error('[mock-harness] failed to create conversation', error);
    ...
    if (requestUrl.pathname.indexOf('/v1/conversations/') === 0 && req.method === 'GET') {
        ...
        try {
            await ensurePersistenceReady();
            const conversation = await getSql('SELECT id, user_id, title, created_at, updated_at FROM conversations WHERE id = ?', [conversationId]);
            if (!conversation) {
...
async function handleFrame(socket, rawFrame, connectionContext) {
    let payload;
    try {
    ...
    switch (payload.type) {
        case 'hello':
            logger.info('[mock-harness] hello version=%s capabilities=%s', payload.hostProtocolVersion, Array.isArray(payload.capabilities) ? payload.capabilities.join(',') : 'none');
            // reply with a minimal assistant_response so the UI can show a connected agent
            try {
                const convoId = sanitizeConversationId(payload.conversationId) || createConversationId();
                const resp = {
        ...
        case 'user_message':
            ...
            return;
        default:
            sendError(socket, sanitizeConversationId(payload.conversationId), 'unsupported_type', 'Unsupported message type.');
    }
}
async function handleCannedUserMessage(socket, payload, connectionContext) {
    ...
    if (cannedFieldQuestion) {
        logger.info('[mock-harness] institute field repair conversationId=%s field=%s instituteStatus=%s', conversationId, cannedFieldQuestion, instituteContext && instituteContext.status ? instituteContext.status : '(none)');
        await sendAssistantResponseForTurn(socket, connectionContext.userId, payload, {
            conversationId,
            correlationId: null,
            messages: [
    ...
    if (viewSummary) {
        ...
        logger.debug('[mock-harness] canned view-qa source=%s rowCount=%s nameMatched=%s dateConstrained=%s dateMatched=%s', 'fallback_summary', viewQa ? viewQa.rowCount : 0, viewQa ? viewQa.nameMatched : false, viewQa ? viewQa.dateConstrained : false, viewQa ? viewQa.dateMatched : false);
        await sendAssistantResponseForTurn(socket, connectionContext.userId, payload, {
            conversationId,
            correlationId: null,
    ...
    if (feeAnswer) {
        const viewQa = inspectViewQa(text, viewContext);
        logger.debug('[mock-harness] canned view-qa source=%s rowCount=%s nameMatched=%s dateConstrained=%s dateMatched=%s', 'fallback_fees', viewQa ? viewQa.rowCount : 0, viewQa ? viewQa.nameMatched : false, viewQa ? viewQa.dateConstrained : false, viewQa ? viewQa.dateMatched : false);
        await sendAssistantResponseForTurn(socket, connectionContext.userId, payload, {
            conversationId,
            correlationId: null,
...
async function handleLiveUserMessage(socket, payload, connectionContext) {
    const conversationId = sanitizeConversationId(payload.conversationId);
    const text = typeof payload.text === 'string' ? payload.text.trim() : '';
    const endpoint = (0, live_client_1.resolveChatCompletionsUrl)(config.live.baseUrl);
    if (!text) {
        sendError(socket, conversationId, 'bad_request', 'user_message.text must be a non-empty string.');
        return;
    }
    ...
    if (!config.live.apiKey || !config.live.model) {
        logger.error('[mock-harness] live turn conversationId=%s path=live endpoint=%s model=%s status=%s latencyMs=%s mapping=%s message=%s', conversationId, endpoint || '(empty)', config.live.model || '(missing)', 0, 0, 'error', 'Live mode is not configured. Set API key and model for the live provider.');
        sendError(socket, conversationId, 'configuration_error', 'Live mode is not configured. Set API key and model for the live provider.');
        return;
    }
    try {
        const currentRoute = readCurrentRoute(payload);
        const instituteContext = readInstituteContext(payload);
        logger.info('[mock-harness] live turn conversationId=%s path=live endpoint=%s model=%s instituteStatus=%s', conversationId, endpoint || '(empty)', config.live.model, instituteContext && instituteContext.status ? instituteContext.status : '(none)');
        const moduleFlags = readModuleFlags(payload);
        const contextFlags = readContextFlags(payload);
        const viewContext = readViewContext(payload);
        const formOptions = readFormOptionsContext(payload);
        const liveResult = await (0, live_client_1.callOpenAICompatibleChat)({
        ...
        logger.info('[mock-harness] live turn conversationId=%s path=live endpoint=%s model=%s status=%s latencyMs=%s mapping=%s', conversationId, liveResult.endpoint, liveResult.model, liveResult.statusCode, liveResult.latencyMs, 'success');
        logger.debug('[mock-harness] live turn debug conversationId=%s responseBytes=%s finishReason=%s', conversationId, liveResult.responseBytes, liveResult.finishReason || '(none)');
        const repaired = mapLiveOutputToAssistant(text, currentRoute, liveResult.text, instituteContext, viewContext, moduleFlags, contextFlags, formOptions);
        if (repaired.debug && repaired.debug.active) {
            logger.debug('[mock-harness] live view-qa conversationId=%s source=%s rowCount=%s nameMatched=%s dateConstrained=%s dateMatched=%s', conversationId, repaired.debug.source, repaired.debug.rowCount, repaired.debug.nameMatched, repaired.debug.dateConstrained, repaired.debug.dateMatched);
        }
        ...
        const liveRepaired = applyLiveTurnRepairs(text, currentRoute, repaired.messages, repaired.actions, instituteContext, initialSurface, conversationId, formOptions);
        ...
        const safeMessage = hasDetails
            ...
            : (error && error.message ? error.message : 'Unknown live provider failure.');
        ...
        logger.error('[mock-harness] live turn conversationId=%s path=live endpoint=%s model=%s status=%s latencyMs=%s mapping=%s message=%s', conversationId, failedEndpoint, failedModel, statusCode, latencyMs, 'error', safeMessage);
        logger.debug('[mock-harness] live turn debug conversationId=%s responseBytes=%s finishReason=%s', conversationId, responseBytes, '(none)');
        if (hasDetails) {
            logger.debug('[mock-harness] live turn error-details conversationId=%s details=%s', conversationId, safeStringifyDebug(liveError.details));
        }
        else if (error && error.stack) {
            logger.debug('[mock-harness] live turn error-stack conversationId=%s stack=%s', conversationId, error.stack);
        }
        sendError(socket, conversationId, 'internal_error', 'Live response is unavailable right now. Please try again shortly.');
    }
}
function isFeesNavigationIntent(text) {
    const normalized = text.toLowerCase();
    const hasFees = /\bfees?\b/.test(normalized);
    return hasFees && hasNavigationPhrase(normalized);
}
function isFeesTransactionsNavigationIntent(text) {
    const normalized = text.toLowerCase();
...
function isCollectFeesIntent(text) {
    const normalized = text.toLowerCase();
    return /\bcollect\b/.test(normalized) && /\bfees?\b/.test(normalized);
}
function isInstituteInfoIntent(text) {
    const normalized = text.toLowerCase();
    if (!normalized.trim()) {
        return false;
    }
...
function isStudentEnrolIntent(text) {
...
function isBuddyWorkspaceIntent(text) {
    const normalized = String(text || '').trim().toLowerCase();
    if (!normalized) {
        return false;
    }
...
function isFormSurfaceIntent(text) {
    return isStudentEnrolIntent(text) || isOrgAddIntent(text);
}
...
function overwriteSurfaceCourseOptions(surface, formOptions) {
    ...
    const container = type === 'student-enrol-form'
        ? (surface.schema && typeof surface.schema === 'object' ? surface.schema : null)
        : surface;
    if (!container || !Array.isArray(container.fields)) {
        return surface;
    }
    const fields = container.fields.map((field) => {
        if (!field || field.id !== 'courseId') {
            return field;
        }
        return Object.assign({}, field, { options: courseOptions });
    });
    const next = Object.assign({}, surface);
...
function surfaceTypeMatches(surface, expected) {
    return !!surface
        && typeof surface === 'object'
        && String(surface.type || surface.kind || '')
            === String(expected.type || expected.kind || '');
}
function isInstitutePageNavigationIntent(text) {
    const normalized = text.toLowerCase();
    if (!normalized.trim()) {
        return false;
    }
...
function isMarksCardIntent(text) {
    const normalized = text.toLowerCase();
    const hasMarksCard = (normalized.indexOf('marks card') >= 0 ||
        normalized.indexOf('marks cards') >= 0 ||
        normalized.indexOf('markscard') >= 0);
    const hasSendOrEmail = (normalized.indexOf('send') >= 0 ||
...
function defaultLiveSystemPrompt() {
    return [
...
function mapLiveOutputToAssistant(userText, currentRoute, modelText, instituteContext, viewContext, moduleFlags, contextFlags, formOptions) {
...
function applyLiveTurnRepairs(userText, currentRoute, messages, actions, instituteContext, surface, conversationId, formOptions) {
    ...
    if (field) {
        logger.info('[mock-harness] institute field repair conversationId=%s field=%s instituteStatus=%s', conversationId, field, instituteContext && instituteContext.status ? instituteContext.status : '(none)');
        return {
            messages: ensureSuggestionChips([createTextBlock(buildInstituteFieldAnswer(userText, instituteContext))], defaultInstituteFollowUpSuggestions(), userText, conversationId),
            actions: [],
            surface: undefined
        };
    }
    let nextSurface = surface;
    const repairedStudentSurface = isStudentEnrolIntent(userText)
    ...
    if (isInstituteInfoIntent(userText) && hasOkInstituteData(instituteContext)) {
        ...
        nextMessages = ensureSuggestionChips(nextMessages, defaultInstituteFollowUpSuggestions(), userText, conversationId);
    ...
    const nextSurfaceType = nextSurface && typeof nextSurface === 'object'
        ? String(nextSurface.type || nextSurface.kind || '')
        : '';
    if (repairedStudentSurface || repairedOrgSurface
...
function withFormOpenSuggestions(messages, userText, conversationId) {
...
function detectClearNavigationKind(text) {
    if (isFeesTransactionsNavigationIntent(text)) {
        return 'transactions';
    }
    if (isCollectFeesIntent(text)) {
...
function repairClearNavigation(kind, currentRoute, messages, actions, userText, conversationId, surface) {
    const spec = {
        fees: {
    ...
    return repairNavigationIntent(kind, spec.route, spec.opening, spec.already, currentRoute, messages, actions, userText, conversationId, spec.chips);
}
function looksLikeInstituteDump(messages, surface) {
    const surfaceType = surface && typeof surface === 'object' ? String(surface.type || surface.kind || '') : '';
...
function repairNavigationIntent(kind, targetRoute, openingText, alreadyText, currentRoute, messages, actions, userText, conversationId, chips) {
    ...
    logger.info('[mock-harness] nav repair conversationId=%s kind=%s alreadyOnRoute=%s injectedText=%s injectedNavigate=%s', conversationId, kind, alreadyThere, true, needsNavigate);
    ...
    let nextActions = actions.slice();
    if (alreadyThere) {
        nextActions = nextActions.filter((action) => action.type !== 'navigate');
    }
    else {
...
function hasNavigateActionTo(actions, route) {
    const target = normalizeRoute(route);
    return actions.some((action) => {
        return action && action.type === 'navigate' && normalizeRoute(action.route) === target;
    });
}
...
function defaultInstituteFollowUpSuggestions() {
    return [
...
function ensureSuggestionChips(messages, chips, userText, conversationId) {
...
function polishInstituteSummaryText(messages, instituteContext, conversationId) {
...
function detectInstituteFieldQuestion(text) {
    const normalized = String(text || '').toLowerCase();
...
function buildInstituteFieldAnswer(text, instituteContext) {
...
function parseStructuredLiveOutput(modelText) {
    ...
    rawMessages.forEach((raw) => {
        ...
        if (raw.type === 'suggestions' && Array.isArray(raw.items)) {
            const items = raw.items
                .map((rawItem) => sanitizeSuggestionItem(rawItem))
                .filter((item) => !!item);
...
function sanitizeSuggestionItem(rawItem) {
...
function applyIntentRepairs(userText, currentRoute, messages, actions, instituteContext, viewContext, moduleFlags, contextFlags, formOptions) {
    ...
    if (isCollectFeesIntent(userText)) {
        ...
        repairedMessages.push(createTextBlock(isUnderFees
            ? 'Opening the payment recording screen in Fees.'
            : 'Fee collection is done in Fees. Opening the add payment screen so you can record a payment there.'));
    ...
    if (isMarksCardIntent(userText)) {
        if (!hasTextLikeMessage(repairedMessages)) {
            ...
            repairedMessages.unshift(createTextBlock(mentionsFa1
                ? 'Marks cards are sent from Connect. Opening the marks-card recipients screen so you can continue with FA1 selection there. (Demo: no email is sent yet.)'
                : 'Marks cards are sent from Connect. Opening the marks-card recipients screen. (Demo: no email is sent yet.)'));
        }
...
function createSuggestionsBlock(items) {
    return {
        type: 'suggestions',
...
function normalizeSuggestionBlocks(messages) {
    const result = [];
    const items = [];
    messages.forEach((block) => {
        if (block.type === 'suggestions' && Array.isArray(block.items)) {
...
function removeSuggestionBlocks(messages) {
...
function dedupeAndCapSuggestions(items) {
    const seen = new Set();
...
function removeRedundantNavigateSuggestions(messages, actions) {
    const navigatedRoutes = new Set(actions
        .filter((action) => action && action.type === 'navigate')
        .map((action) => normalizeRoute(action.route))
        .filter((route) => !!route));
    if (!navigatedRoutes.size) {
        return messages;
    }
    return messages.reduce((result, block) => {
        if (block.type !== 'suggestions' || !Array.isArray(block.items)) {
            result.push(block);
            return result;
        }
        const items = dedupeAndCapSuggestions(block.items).filter((item) => {
...
function createNavigateAction(route) {
...
function appendNavigateAction(actions, route) {
    const normalizedRoute = normalizeRoute(route);
    if (!isAllowedRoute(normalizedRoute)) {
        return;
    }
    if (actions.some((action) => action && action.type === 'navigate' && normalizeRoute(action.route) === normalizedRoute)) {
        return;
    }
    actions.push(createNavigateAction(normalizedRoute));
}
function createSendMessageSuggestion(label, text) {
    return {
...
function suggestionPromptForRoute(href, label) {
...
function defaultSafeFallbackText() {
    return 'I can open Fees, Institute details, marks-card recipients in Connect, and Home.';
}
function buildUnknownNavigationFallbackBlocks() {
    return [
        createTextBlock('I can\'t open that destination. Try asking for Fees, Home, institute info, or marks cards.'),
        createSuggestionsBlock([
...
function hasTextLikeMessage(messages) {
    return messages.some((message) => {
        if (message.type === 'text') {
            return typeof message.text === 'string' && message.text.trim().length > 0;
        }
        if (message.type === 'markdown') {
            return typeof message.markdown === 'string' && message.markdown.trim().length > 0;
        }
        return false;
    });
}
function hasOnlyLowSignalTextLikeMessages(messages) {
    const textLikeMessages = messages.filter((message) => {
        return message.type === 'text' || message.type === 'markdown';
    });
    if (!textLikeMessages.length) {
        return false;
    }
    return textLikeMessages.every((message) => {
...
function hasInternalLinkForRoute(messages, route) {
    const normalizedRoute = normalizeRoute(route);
    return messages.some((message) => {
...
function isAllowedRoute(route) {
    ...
    return KNOWN_NAVIGATION_ROUTES.some((knownRoute) => {
...
function extractTextFallback(value) {
...
function looksLikeProtocolJsonBlob(text) {
    if (!text) {
        return false;
    }
    ...
    const startsLikeJson = ((trimmed.charAt(0) === '{' && trimmed.charAt(trimmed.length - 1) === '}')
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            return parsed.some((item) => item && typeof item === 'object' && (item.messages || item.actions || item.type === 'assistant_response'));
        }
...
async function handleEmitEvent(socket, payload) {
    const conversationId = sanitizeConversationId(payload.conversationId);
...
function emitEventLivePath(payload) {
...
function sendCannedEmitEventAck(socket, payload, conversationId, correlationId) {
...
async function tryLiveEmitEventAck(socket, payload, conversationId, correlationId, livePath) {
    const endpoint = (0, live_client_1.resolveChatCompletionsUrl)(config.live.baseUrl);
    if (!config.live.apiKey || !config.live.model) {
        logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, livePath, 'error', 'Live mode is not configured; falling back to canned ack.');
        return false;
    }
    ...
    try {
        logger.info('[mock-harness] emit_event conversationId=%s path=%s endpoint=%s model=%s', conversationId, livePath, endpoint || '(empty)', config.live.model);
        const liveResult = await (0, live_client_1.callOpenAICompatibleChat)({
            baseUrl: config.live.baseUrl,
            apiKey: config.live.apiKey,
            model: config.live.model,
        ...
        if (!parsed || !hasTextLikeMessage(parsedMessages)) {
            logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, livePath, 'error', 'Live emit_event response was empty or unparseable; falling back to canned ack.');
            return false;
        }
        const mapped = mapLiveOutputToAssistant(userText, readCurrentRoute(contextPayload), liveResult.text, readInstituteContext(contextPayload), readViewContext(contextPayload), readModuleFlags(contextPayload), readContextFlags(contextPayload));
        const eventName = typeof payload.event === 'string' ? payload.event.trim() : '';
        const actions = eventName === 'form_submit' ? [] : mapped.actions;
        logEmitEvent(payload, conversationId, livePath);
        logger.info('[mock-harness] emit_event conversationId=%s path=%s endpoint=%s model=%s status=%s latencyMs=%s mapping=%s', conversationId, livePath, liveResult.endpoint, liveResult.model, liveResult.statusCode, liveResult.latencyMs, 'success');
        sendAssistantResponse(socket, {
            conversationId,
            correlationId,
        ...
        const safeMessage = hasDetails
            ? liveError.details.safeMessage
            : (error && error.message ? error.message : 'Unknown live provider failure.');
        logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, livePath, 'error', safeMessage);
        if (hasDetails) {
            logger.debug('[mock-harness] emit_event error-details conversationId=%s details=%s', conversationId, safeStringifyDebug(liveError.details));
        }
        return false;
    }
}
function emitEventAsContextPayload(payload) {
    return {
        type: 'user_message',
...
function buildEmitEventLiveUserText(payload, conversationId, contextPayload) {
    const eventName = typeof payload.event === 'string' ? payload.event.trim() : '';
...
function logEmitEvent(payload, conversationId, path) {
    const context = payload.context || { route: '' };
    ...
    const focusType = context.focus && typeof context.focus.type === 'string'
        ? String(context.focus.type)
        : '(none)';
    ...
    const underStudent = typeof moduleFlags.underStudent === 'boolean'
        ...
        : '(unknown)';
    const viewModule = context.view && typeof context.view.module === 'string'
        ? String(context.view.module)
        : '(none)';
    const viewScreen = context.view && typeof context.view.screen === 'string'
        ? String(context.view.screen)
        : '(none)';
    const viewRows = context.view && Array.isArray(context.view.rows)
        ? String(context.view.rows.length)
        : '0';
    logger.info('[mock-harness] emit_event conversationId=%s path=%s route=%s persona=%s focus=%s underFees=%s underConnect=%s underStudent=%s viewModule=%s viewScreen=%s viewRows=%s', conversationId, path, route, persona, focusType, underFees, underConnect, underStudent, viewModule, viewScreen, viewRows);
}
function ackTextForEmitEvent(payload) {
    const eventName = typeof payload.event === 'string' ? payload.event.trim() : '';
    const action = typeof payload.action === 'string' ? payload.action.trim() : '';
...
function isBuddyParseEvent(payload) {
    const eventName = typeof payload.event === 'string' ? payload.event.trim() : '';
    const action = typeof payload.action === 'string' ? payload.action.trim() : '';
...
function sanitizeCannedFieldValue(fieldId, value) {
    if (fieldId === 'dob') {
...
function cannedSanitizeSlotIndex(fieldId) {
    if (fieldId === 'dob') {
        return 1;
    }
    if (fieldId === 'gender') {
...
function removeCannedSpan(text, start, end) {
    return (text.slice(0, start) + text.slice(end)).replace(/\s{2,}/g, ' ').trim();
}
function stripCannedEdgePunct(token) {
...
function scanCannedShapeTokens(segment, slots) {
...
function sanitizeCannedChunk(rawChunk) {
...
function splitCannedStudentChunks(line) {
...
function sanitizeCannedStudentLine(line) {
...
function cannedBuddySanitize(rawText) {
    const original = String(rawText || '');
    const outLines = [];
    let labeledSlots = null;
    const flushLabeled = () => {
        if (labeledSlots) {
            outLines.push(labeledSlots.join(', ')
                .replace(/^(?:\s*,\s*)+/, '')
                .replace(/(?:\s*,\s*)+$/, ''));
            labeledSlots = null;
        }
...
async function handleBuddyParseEvent(socket, payload, conversationId, correlationId) {
...
// ============================================================================
function readSanitizeAgentPrompt() {
    try {
...
function buildBuddySanitizeChildUserText(buddyText, fields) {
    return [
...
function validateBuddySanitizedText(modelText, maxSlots) {
...
async function tryBuddySanitizeChild(payload, conversationId, buddyText, fields) {
    ...
    try {
        ...
        if (!sanitized) {
            logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, 'buddy-sanitize-child-fallback', 'error', 'Sanitizer output was empty, non-plain-text, or not schema lines.');
            return null;
        }
        logEmitEvent(payload, conversationId, 'buddy-sanitize-child');
        logger.info('[mock-harness] emit_event conversationId=%s path=%s endpoint=%s model=%s status=%s latencyMs=%s lines=%s', conversationId, 'buddy-sanitize-child', liveResult.endpoint, liveResult.model, liveResult.statusCode, liveResult.latencyMs, sanitized.split('\n').filter((line) => line.trim()).length);
        return sanitized;
    }
    catch (error) {
        const liveError = error;
        const safeMessage = liveError && liveError.details
            ? liveError.details.safeMessage
            : (error && error.message ? error.message : 'Unknown sanitize child failure.');
        logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, 'buddy-sanitize-child-fallback', 'error', safeMessage);
        return null;
    }
}
...
function countBuddySchemaLines(text) {
...
// ============================================================================
const VALIDATE_AGENT_BYLINE = { id: 'ct-bot-validate', displayName: 'Validate agent' };
function isBuddyValidateEvent(payload) {
    const eventName = typeof payload.event === 'string' ? payload.event.trim() : '';
    const action = typeof payload.action === 'string' ? payload.action.trim() : '';
    return eventName === 'buddy_validate' || action === 'student.enrol.validate';
}
function readBuddyValidatePayload(payload, conversationId) {
    const values = payload.values && typeof payload.values === 'object' ? payload.values : {};
    ...
    return {
        ...
        parse: values.parse && typeof values.parse === 'object' ? values.parse : undefined
    };
}
...
function cannedBuddyValidateRecords(input) {
    const results = [];
    input.records.forEach((record) => {
        ...
        input.fields.forEach((field) => {
            ...
            const staleRejection = rejected.filter((item) => item && String(item.fieldId || '').trim().toLowerCase() === field.id.toLowerCase()
            ...
            const selectUnresolved = field.type === 'select' && !!value
                ...
                && !field.options.some((option) => option.value.toLowerCase() === value.toLowerCase() || option.label.toLowerCase() === value.toLowerCase());
            if (dateInvalid) {
...
function readValidateAgentPrompt() {
    try {
...
function buildBuddyValidateChildUserText(input) {
    return [
        ...
        'records (id, values, labels, rejected): ' + JSON.stringify(input.records.map((record) => ({
            id: record.id,
...
function parseBuddyValidateChildOutput(modelText, input) {
    const parsed = parseJsonFromModelText(modelText);
    ...
    raw.forEach((item) => {
        ...
        const reasons = Array.isArray(item.reasons)
            ? item.reasons.filter((reason) => typeof reason === 'string' && reason.trim()).slice(0, 10)
            : [];
...
function replyBuddyValidate(socket, payload, conversationId, correlationId, input, results) {
    const byId = {};
    results.forEach((result) => {
        byId[result.id] = result;
    });
    const appliedRecords = input.records.map((record) => {
    ...
    const invalidCount = appliedRecords.filter((record) => record.valid === false).length;
    ...
    const surface = buildBuddyWorkspaceSurface(input.buddyText, appliedRecords, input.parse, courseOptionsFromUserMessage(emitEventAsContextPayload(payload)), input.fields);
    if (input.surfaceId) {
 ...
 * canned rules). */
async function tryBuddyValidateChild(payload, conversationId, input) {
    ...
    try {
        ...
        if (!results) {
            logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, 'buddy-validate-child-fallback', 'error', 'Child output was empty, non-JSON, or missing records.');
            return null;
        }
        logEmitEvent(payload, conversationId, 'buddy-validate-child');
        logger.info('[mock-harness] emit_event conversationId=%s path=%s endpoint=%s model=%s status=%s latencyMs=%s records=%s', conversationId, 'buddy-validate-child', liveResult.endpoint, liveResult.model, liveResult.statusCode, liveResult.latencyMs, results.length);
        return results;
    }
    catch (error) {
        const liveError = error;
        const safeMessage = liveError && liveError.details
            ? liveError.details.safeMessage
            : (error && error.message ? error.message : 'Unknown validate child failure.');
        logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, 'buddy-validate-child-fallback', 'error', safeMessage);
        return null;
    }
}
async function handleBuddyValidateEvent(socket, payload, conversationId, correlationId) {
...
function buildBuddyParseChildUserText(buddyText, fields, courseOptions) {
    return [
...
async function tryBuddyParseChild(socket, payload, conversationId, correlationId, buddyText, courseOptions, sanitizeChatPrefix, fields) {
    ...
    try {
        ...
        if (!child) {
            logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, 'buddy-parse-child-fallback', 'error', 'Child output was empty, non-JSON, or failed span validation.');
            return false;
        }
        logEmitEvent(payload, conversationId, 'buddy-parse-child');
        logger.info('[mock-harness] emit_event conversationId=%s path=%s endpoint=%s model=%s status=%s latencyMs=%s records=%s', conversationId, 'buddy-parse-child', liveResult.endpoint, liveResult.model, liveResult.statusCode, liveResult.latencyMs, child.records.length);
        ...
        const safeMessage = liveError && liveError.details
            ? liveError.details.safeMessage
            : (error && error.message ? error.message : 'Unknown parse child failure.');
        logger.error('[mock-harness] emit_event conversationId=%s path=%s mapping=%s message=%s', conversationId, 'buddy-parse-child-fallback', 'error', safeMessage);
        return false;
    }
}
...
function parseBuddyChildOutput(modelText, buddyText, courseOptions) {
    const parsed = parseJsonFromModelText(modelText);
    if (!parsed || typeof parsed !== 'object') {
        return null;
    }
...
function isValidBuddySpan(span, textLength) {
...
function sanitizeBuddyChildRecord(raw, index, buddyText, courseOptions) {
    if (!raw || typeof raw !== 'object') {
        return null;
    }
    ...
    const values = { name: '', dob: '', gender: '', courseId: '' };
    const rawValues = raw.values && typeof raw.values === 'object' ? raw.values : {};
    ['name', 'dob', 'gender', 'courseId'].forEach((fieldId) => {
        const value = rawValues[fieldId];
    ...
    const leftovers = (Array.isArray(raw.leftovers) ? raw.leftovers : [])
 ...
 * re-splitting text. */
function cannedBuddyParse(rawText, courseOptions) {
    const records = [];
    const parseMapped = [];
    const parseRejected = [];
    const parseLeftovers = [];
...
function classifyCannedSegment(record, segment, tokenSpan, parseRejected, courseOptions) {
    const labeled = matchLabeledSegment(segment);
    if (labeled) {
        // Span only the value part of "key: value", not the key.
        const keyLength = matchLabeledKeyLength(segment);
        const valueStart = keyLength >= 0 ? keyLength : 0;
        assignCannedField(record, labeled.fieldId, labeled.value, { start: tokenSpan.start + valueStart, end: tokenSpan.start + valueStart + labeled.value.length }, parseRejected, courseOptions);
        return;
    }
...
function matchLabeledSegment(segment) {
...
function parseCannedGender(segment) {
...
function parseCannedCourse(segment, courseOptions) {
    const text = segment.trim();
    if (!text) {
...
function looksLikeCannedName(segment) {
...
function assignCannedField(record, fieldId, value, span, parseRejected, courseOptions) {
    const text = String(value || '').trim();
    if (!text) {
        return;
    }
...
const liveBuddyFieldsByConversation = new Map();
...
const lastBuddyNotesByConversation = new Map();
function rememberBuddyNotes(conversationId, buddyText) {
...
function rememberedBuddyNotes(conversationId) {
...
function sanitizeBuddyFieldsList(rawFields) {
    if (!Array.isArray(rawFields)) {
        return [];
    }
    const fields = [];
    rawFields.slice(0, 20).forEach((item) => {
        if (item && typeof item === 'object'
            && typeof item.id === 'string' && item.id.trim()
            && typeof item.label === 'string' && item.label.trim()
            && typeof item.type === 'string' && item.type.trim()) {
            const field = { id: item.id.trim(), label: item.label.trim(), type: item.type.trim() };
...
function rememberBuddyFieldsList(conversationId, rawFields) {
    ...
    logger.info('[mock-harness] live workspace fields conversationId=%s fields=%s', conversationId, fields.map((field) => field.id).join(','));
    return fields;
}
function rememberBuddyFieldsFromSurface(conversationId, surface) {
    if (!surface || typeof surface !== 'object') {
        return;
    }
    if (String(surface.type || '') !== 'buddy-enrol-workspace') {
        return;
    }
    rememberBuddyFieldsList(conversationId, surface.fields);
}
function rememberedBuddyFields(conversationId) {
    return liveBuddyFieldsByConversation.get(conversationId) || [];
}
function buildBuddyWorkspaceSurface(buddyText, records, parse, courseOptions, fieldsOverride) {
    ...
    const surfaceFields = Array.isArray(fieldsOverride)
        ? fieldsOverride.map((field) => (field.type === 'select' && field.id === 'courseId'
...
function readContextFlags(payload) {
    const context = payload.context || { route: '' };
    const flags = context && context.flags;
    if (!flags || typeof flags !== 'object' || Array.isArray(flags)) {
        return {};
    }
    return flags;
}
function readModuleFlags(payload) {
    const context = payload.context || { route: '' };
    const moduleFlags = context && context.moduleFlags;
    if (!moduleFlags || typeof moduleFlags !== 'object' || Array.isArray(moduleFlags)) {
        return {};
    }
    return moduleFlags;
}
function readViewContext(payload) {
    const context = payload.context || { route: '' };
...
function isFeesViewContext(viewContext) {
    return !!(viewContext && typeof viewContext.module === 'string' && viewContext.module.toLowerCase() === 'fees');
}
function readInstituteContext(payload) {
    const context = payload.context || { route: '' };
    const institute = context && context.institute;
    if (!institute || typeof institute !== 'object' || Array.isArray(institute)) {
        return null;
    }
    return institute;
}
function isViewSummaryQuestion(text) {
    const normalized = (text || '').toLowerCase();
    if (!normalized.trim()) {
        return false;
    }
...
function summarizeVisibleView(text, viewContext) {
...
function hasNotFoundLikeTextMessage(messages) {
    return messages.some((message) => {
        if (message.type !== 'text') {
            return false;
        }
        return isNotFoundLikeText(message.text);
    });
}
function isNotFoundLikeText(text) {
...
function isUnresolvedViewAnswer(text) {
    const normalized = normalizeSearchText(text || '');
    if (!normalized) {
        return true;
    }
...
function inspectViewQa(text, viewContext) {
    ...
    const nameMatchedRows = rows.filter((row) => {
    ...
    if (askedDate && dateKey) {
        dateMatched = nameMatchedRows.some((row) => normalizeDateToken(row[dateKey]) === askedDate);
    }
...
function isFeesAmountQuestion(text) {
    const normalized = (text || '').toLowerCase();
    if (!normalized.trim()) {
        return false;
    }
...
function answerFeesAmountQuestion(text, viewContext) {
    ...
    const totals = firstRows.reduce((acc, row) => {
...
function extractCandidateStudentName(text) {
...
function readViewRows(viewContext) {
    return Array.isArray(viewContext.rows) ? viewContext.rows : [];
}
function readViewColumns(viewContext, rows) {
...
function resolveFirstMatchingKey(columns, patterns) {
...
function summarizeSampleRows(rows, columns) {
    if (!rows.length) {
        return '';
    }
    const preferredColumns = columns.slice(0, 6);
    const sampleRows = rows.slice(0, 2).map((row, index) => {
        const cells = [];
...
function assistantClaimsInstituteUnavailable(messages) {
    const text = messages
        .map((message) => {
        if (message.type === 'text') {
            return String(message.text || '');
...
function replaceFirstTextLikeMessage(messages, text) {
    let replaced = false;
...
function repairInstituteUnavailableText(messages, instituteContext, surface, conversationId) {
    const surfaceData = surface && surface.type === 'institute-summary' && surface.data
        ? surface.data
        : null;
    ...
    const name = readDisplayValue((summaryContext && summaryContext.data && summaryContext.data.name) || (surfaceData && surfaceData.name));
    ...
    logger.info('[mock-harness] institute text repair conversationId=%s reason=unavailable-or-missing-summary-with-ok-data', conversationId);
    return replaceFirstTextLikeMessage(messages, buildInstituteSummaryMessage(summaryContext, false));
}
function buildInstituteSummaryMessage(instituteContext, allowDemoFallback) {
    if (instituteContext && instituteContext.status === 'ok' && instituteContext.data && typeof instituteContext.data === 'object') {
        const data = instituteContext.data;
        const lines = ['Institute summary:'];
...
function buildInstituteSurface(instituteContext) {
...
function readFormOptionsContext(payload) {
...
function courseOptionsFromUserMessage(payload) {
    const formOptions = readFormOptionsContext(payload);
    return mapSelectOptions(formOptions && formOptions.courses);
}
function mapSelectOptions(raw) {
    ...
    for (let i = 0; i < raw.length && options.length < 100; i++) {
        ...
        const valueSource = item.value != null ? item.value : item.id;
        const value = valueSource == null ? '' : String(valueSource).trim();
        const labelSource = item.label != null ? item.label : item.name;
...
function normalizeSelectLabel(label) {
    return String(label || '').trim().replace(/\s+/g, ' ').toLowerCase();
}
function studentEnrolOpenMessage(formOptions) {
...
function orgAddOpenMessage(formOptions) {
...
function buildStudentEnrolFormSurface(formOptions) {
...
function buildOrgAddFormSurface(formOptions) {
...
function formatInstituteAddress(value) {
    ...
    ].map(readDisplayValue).filter((part) => !!part).join(', ');
}
...
function formatAcademicYear(fromValue, toValue) {
...
async function sendAssistantResponseForTurn(socket, userId, userMessage, payload) {
    ...
    logger.info('[mock-harness] assistant response hasSurface=%s surfaceType=%s', !!response.surface, response.surface && response.surface.type ? response.surface.type : '(none)');
    ...
    if (!persistedUserTurn) {
        logger.error('[mock-harness] skipped assistant persistence because user turn was not persisted conversationId=%s user=%s', conversationId, userId);
...
function filterSuggestionsForTurn(messages) {
    return messages.reduce((result, block) => {
        if (block.type !== 'suggestions' || !Array.isArray(block.items)) {
            result.push(block);
            return result;
        }
        const incoming = block.items;
        const items = dedupeAndCapSuggestions(incoming).filter((item) => {
        ...
        if (dropped > 0) {
            logger.info('[mock-harness] suggestion filter dropped=%s kept=%s', dropped, items.length);
        }
        if (items.length) {
            result.push(Object.assign({}, block, { items }));
        }
...
const ALLOWLISTED_SUGGESTION_TEXTS = [
    'take me to fees',
    'open fees',
    'collect fees',
...
function isAllowlistedSuggestionText(text) {
    const normalized = normalizeSuggestionText(text);
    if (!normalized) {
        return false;
    }
    return ALLOWLISTED_SUGGESTION_TEXTS.some((allowed) => {
        return normalizeSuggestionText(allowed) === normalized;
    });
}
function assistantTextRefusesNavigation(messages) {
    const text = messages
        .map((message) => {
        if (message.type === 'text') {
            return String(message.text || '');
...
function sendAssistantResponse(socket, payload) {
    socket.send(JSON.stringify(composeAssistantResponse(payload)));
}
function sendError(socket, conversationId, code, message) {
    const response = {
        type: 'error',
        conversationId,
        code,
...
function safeStringifyDebug(value) {
    try {
...
function writeCors(res, statusCode, corsHeaders, extraHeaders = {}) {
...
httpServer.listen(config.port, config.host, () => {
    ...
    logger.info('[mock-harness] protocol=%s capabilities=%s', ct_bot_host_protocol_1.hostProtocolVersion, ct_bot_host_protocol_1.hostProtocolCapabilities.join(','));
    ...
    if (config.mode === 'live') {
        logger.debug('[mock-harness] live.chatCompletionsUrl=%s', (0, live_client_1.resolveChatCompletionsUrl)(config.live.baseUrl));
    }
});
